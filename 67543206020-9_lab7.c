    ...
    while ( 1 ){ //#01
        ...
        if ( in_game == 1 )
        {
            // randomNumber = rand( ) % 101 ; //สุ่มตัเลข
             randomNumber = 42 ; //สุ่มตัเลข
            printf( "(Score=%d)\n", Score ) ;
            // printf( "Random number : %d\n", randomNumber ) ;
            do{
                printf( "Guess the winning number (%d-%d): ", dif1, dif2  ) ;
                scanf( "%d", &user_number ) ;
                if ( user_number < dif1 || user_number > dif2 )
                {
                    printf("Your guess is out of the current bounds (%d-%d)! Try again.\n", dif1, dif2);
                    continue ;  // กลับไปให้ผู้ใช้กรอกเลขใหม่
                } 
                
                if ( user_number == randomNumber ){//ตอบถูก
                    printf( "That is correct! The winning number is %d.\n", randomNumber ) ;
                    printf( "Score this game: %d\n", Score ) ;
                }else if ( user_number < randomNumber ){//more than
                    Score -= 10 ;
                    ...
                    // dif1 = user_number + 1 ;
                }else if ( user_number > randomNumber ){//less
                    Score -= 10 ;
